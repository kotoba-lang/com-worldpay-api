(ns worldpay_api.main
  "Kotodama WASM entrypoint for the Worldpay_api clean-room actor (L4) — Clojure port.
  ...
  facts (`worldpay_api.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [kotoba.lang.text :as str]))

(def ns-prefix "worldpay_api")
(def tier "L4")
...
(defn emit-facts
  "EAVT facts for one record: {\"worldpay_api.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "worldpay_api-compat" :tier tier :entities entities} 200])
...
(defn start! [] :worldpay_api-compat/ready)
